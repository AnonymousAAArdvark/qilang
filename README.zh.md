...
<h2 align="center">气 (Qi) 语</h3>
  ...
  <p align="center">
    ...
    <br /><br />
    <a href="https://github.com/anonymousaaardvark/qilang/blob/master/README.md">English</a> |
    中文
  </p>
</div>
...
基准测试的代码可以在[test 文件夹](https://github.com/anonymousaaardvark/qilang/tree/master/test/benchmark )中找到。
...
## 单元测试
单元测试非常重要（特别是对于编程语言），我已经包含了一个完整的气测试套件，以确保解释器的每个方面都做它应该做的事情。测试程序可以在```/test```目录中找到[这里]( https://github.com/anonymousaaardvark/qilang/tree/master/test )。为了自动化测试过程，[这里]( https://github.com/anonymousaaardvark/qilang/blob/master/utils/test.go ) 提供的 Go 程序 ```/utils/test.go``` 运行每个使用输入的解释器测试程序，捕获输出并验证结果。
###运行测试程序
